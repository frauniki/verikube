...
`startingDeadline`(デフォルト 200s)は、逃した tick がどれだけ遅れて発火してよいかを制限します: deadline 内の直近の missed tick は(遅れて)発火し、それより古いものはスキップされます。したがって:

- スイートの suspend を解除しても、suspend していた期間のキャッチアップ run は発火しません(逃してから 200s 以内なら最後の tick のみ発火し得ます)。
